...
const RecipeMaker = () => {
  ...
  const handleGenerateRecipe = async () => { // Made async
    if (!dishInput.trim()) {
    ...
    setIsLoading(true);
    const loadingToastId = showLoading("Sending your request..."); // Updated toast message
    ...
    try {
      const response = await fetch("https://n8nweb.ikusasa.nz/webhook-test/4019b6af-bdeb-43b1-88b4-5cbeece03333", {
        ...
        body: JSON.stringify({ dishInput: dishInput }), // Sending dishInput as JSON
      });
...
Note: For a real application, this recipe would be generated by an AI or a database.
      ...
      setGeneratedRecipe(mockRecipe);
        ...
        showError(`Failed to send request. Status: ${response.status}`);
        setGeneratedRecipe("Failed to generate recipe. Please try again.");
      }
    } catch (error) {
      console.error("Error sending request:", error);
      showError("An error occurred while sending your request.");
