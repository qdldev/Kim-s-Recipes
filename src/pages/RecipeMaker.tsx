...
const RecipeMaker = () => {
  ...
  const handleGenerateRecipe = async () => {
    ...
    setIsLoading(true);
    const loadingToastId = showLoading("Sending your request...");

    try {
      console.log("Sending POST request with dishInput:", dishInput);
      const response = await fetch("https://ikusasanz.app.n8n.cloud/webhook-test/f4035689-8c77-4312-84a6-2372a44bba7d", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ dishInput: dishInput }),
      });
      ...
      if (response.ok) {
        ...
        console.error(`Failed to send request. Status: ${response.status}, Status Text: ${response.statusText}, Response Body: ${errorText}`);
        showError(`Failed to send request. Status: ${response.status}. Please check console for details.`);
        setGeneratedRecipe("Failed to generate recipe. Please try again.");
      }
    } catch (error) {
      console.error("An error occurred during the fetch request:", error);
      showError("An error occurred while sending your request. Please check console for network errors.");
      setGeneratedRecipe("Failed to generate recipe due to a network error. Please try again.");
    } finally {
